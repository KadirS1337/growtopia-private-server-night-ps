...
struct GuildMembers
{
	string growid = "";
	int totalxp = 0;
	long long joinedGuildTime = 0;
	GuildRanks rank = Member;
	bool guildcheckbox_public = true;
	bool guildcheckbox_notifications = true;
};
...
struct GuildInfo
{
	...
	int background = 0;
	int foreground = 0;
	int worldlockidbeforeguildcreation = 242;
	string world = "";
	string statement = "";
	string leader = "";
	string name = "";
	string logs = "";
	string notebook = "";
	vector<GuildMembers> members{};
	bool needsave = false;
	bool accesslockCoLeader = false;
	bool accesslockCoLeaderAndElder = false;
	bool accesslockAllMembers = false;
};
...
struct WorldItem
{
	...
	vector<string> vipentranceaccess;
	int displayblock = 0;
	int breakLevel = 0;
...
struct WorldInfo
{
	...
	string notification = "";
	bool usedkickall = false;
	long long usedkickalltime = 0;
	string kickallby = "";
...
{
public:
	static AGuild getGuild(string name);
	GuildInfo* get_pointer(string name) const;
	void CreateNewGuild(string name, string realname, string leader, string world, string statement, int worldlockidbeforeguildcreation);
	void RemoveGuild(string name);
	GuildDB();
};
inline GuildDB::GuildDB()
{
}

inline void GuildDB::RemoveGuild(string name)
{
	try {
		guilds.erase(
			std::remove_if(guilds.begin(), guilds.end(), [name](const GuildInfo& a) {
				return stringtolower(a.name) == name;
				}),
			guilds.end());
	}
	catch (std::exception& e) {
		std::cout << e.what() << std::endl;
		cout << "Remove Guild (" << name << ") error!" << endl;
	}
}

inline void GuildDB::CreateNewGuild(string name, string realname, string leader, string world, string statement, int worldlockidbeforeguildcreation)
{
	if (std::experimental::filesystem::exists("save/guilds/_" + name + ".json")) return;
	json j;
	...
	j["background"] = 0;
	j["foreground"] = 0;
	j["worldlockidbeforeguildcreation"] = worldlockidbeforeguildcreation;
	...
	j["logs"] = "";
	j["notebook"] = "";
	j["accesslockCoLeader"] = false;
	j["accesslockCoLeaderAndElder"] = false;
	j["accesslockAllMembers"] = false;
	
	json member;
	member["growid"] = leader;
	member["totalxp"] = 0;
	member["joinedGuildTime"] = GetCurrentTimeInternalSeconds();
	member["rank"] = Leader;
	member["guildcheckbox_public"] = true;
	member["guildcheckbox_notifications"] = true;
	members.push_back(member);
	member.clear();
	j["members"] = members;

	ofstream write_guild("save/guilds/_" + name + ".json");
	...
	gmembers.growid = leader;
	gmembers.totalxp = 0;
	gmembers.joinedGuildTime = GetCurrentTimeInternalSeconds();
	gmembers.rank = Leader;
	info.members.push_back(gmembers);
...
inline AGuild GuildDB::getGuild(string name) {
	AGuild ret;
	try {
		name = getStrUpper(name);
		const auto p = std::find_if(guilds.begin(), guilds.end(), [name](const GuildInfo& a) { return stringtoupper(a.name) == name; });
		if (p != guilds.end()) {
			ret.id = p - guilds.begin();
			ret.info = guilds.at(p - guilds.begin());
			ret.ptr = &guilds.at(p - guilds.begin());
			return ret;
		}

		try {
			ifstream read_world("save/guilds/_" + name + ".json");
			...
			int xp = j["xp"].get<int>();
			int background = j["background"].get<int>();
			int foreground = j["foreground"].get<int>();
			int worldlockidbeforeguildcreation = j["worldlockidbeforeguildcreation"].get<int>();
			string statement = j["statement"].get<string>();
			string leader = j["leader"].get<string>();
			string name = j["name"].get<string>();
			string world = j["world"].get<string>();
			string glogs = j["logs"].get<string>();
			string notebook = j["notebook"].get<string>();
			bool accesslockCoLeader = j["accesslockCoLeader"].get<bool>();
			bool accesslockCoLeaderAndElder = j["accesslockCoLeaderAndElder"].get<bool>();
			bool accesslockAllMembers = j["accesslockAllMembers"].get<bool>();
			json members = j["members"];
			...
			info.accesslockCoLeader = accesslockCoLeader;
			info.accesslockCoLeaderAndElder = accesslockCoLeaderAndElder;
			info.accesslockAllMembers = accesslockAllMembers;
			info.worldlockidbeforeguildcreation = worldlockidbeforeguildcreation;
			for (int i = 0; i < members.size(); i++)
			{
				GuildMembers gmembers;
				gmembers.growid = members.at(i)["growid"].get<string>();
				gmembers.totalxp = members.at(i)["totalxp"].get<int>();
				gmembers.joinedGuildTime = members.at(i)["joinedGuildTime"].get<long long>();
				gmembers.rank = (GuildRanks)members.at(i)["rank"].get<int>();
				gmembers.guildcheckbox_public = (GuildRanks)members.at(i)["guildcheckbox_public"].get<bool>();
				gmembers.guildcheckbox_notifications = (GuildRanks)members.at(i)["guildcheckbox_notifications"].get<bool>();
				info.members.push_back(gmembers);
			}
			guilds.push_back(info);
			j.clear();
			members.clear();
			ret.id = guilds.size() - 1;
			ret.info = info;
			ret.ptr = &guilds.at(guilds.size() - 1);
			return ret;
		}
		catch (std::exception& e) {
			std::cout << e.what() << std::endl;
...
inline GuildInfo* GuildDB::get_pointer(const string name) const
{
	AGuild guildas = this->getGuild(name);
	if (guildas.id == -1) {
		return NULL;
	}
	return guildas.ptr;
...
vector<string> premium_worlds;
vector<string> diamond_worlds;
vector<string> platinum_worlds;
...
inline void GenerateGrowaloneLikeWorld(WorldInfo* world, const bool cave, const int width, const int height, const int dirtType = 2, const int lavaType = 4, const int mainDoorType = 6, const int bedrockType = 8, const int rockType = 10, const int caveBackgroundType = 14, const int wmachineid = 0, const int deco1Id = 0, const int deco2Id = 0, const int id = 0, const bool isTreasureBlastWorld = false) {
	try {
		...
		if (cave) {
			...
			settiletext(world, mainDoorX + 1, f, "Madene Hosgeldin!");
			...
			settile(world, minerX, f, 482);
			settiletext(world, minerX + 1, f, "Madencimiz tasindi, topladiginiz madenleri satmak icin BASLA dunyasina gidiniz!");
			...
			for (auto i = 1; i < width; i++)
			{
				...
				//remove comment if u want world with "mountains"
			...
			if (!isTreasureBlastWorld)
			{
				//for all worlds (except treasure blasted world)
			...
			{
				//only for treasure blasted world
				for (auto srg = height - 18; srg < height - 10; srg++)
				{
					for (auto dhk = 0; dhk < width; dhk++)
					{
						const auto djkf = rand() % 2;
				...
				for (auto srg = height - 40; srg < height - 30; srg++)
				{
					for (auto dhk = 0; dhk < width; dhk++)
					{
						const auto djkf = rand() % 2;
				...
				for (auto srg = height - 60; srg < height - 50; srg++)
				{
					for (auto dhk = 0; dhk < width; dhk++)
					{
						const auto djkf = rand() % 2;
			...
			if (isTreasureBlastWorld)
			{
				auto deco2Count = rand() % 12 + 3;
				for (auto i = 0; i < deco2Count; i++)
				{
					const auto a = rand() % width;
					const auto b = rand() % ((height - 7) - (height - 11) + 1) + (height - 11);// from (height -7) to (height - 11)
					settile(world, a, b, 1538);
				}

				deco2Count = rand() % 12 + 3;
				for (auto i = 0; i < deco2Count; i++)
				{
					const auto a = rand() % width;
					const auto b = rand() % ((height - 7) - (height - 11) + 1) + (height - 11);
					settile(world, a, b, 944);
				}
...
inline WorldInfo CreateWorld(const string name, int width, int height, const bool cave = false, const int dirtType = 2, const int lavaType = 4, const int mainDoorType = 6, const int bedrockType = 8, const int rockType = 10, const int caveBackgroundType = 14, bool isTreasureBlastWorld = false) {
	WorldInfo world;
	world.name = name;
	world.width = width;
	world.height = height;
	world.drop_gem = false;
	GenerateGrowaloneLikeWorld(&world, cave, width, height, dirtType, lavaType, mainDoorType, bedrockType, rockType, caveBackgroundType, 0, 0, 0, BlastProperties_RandomDeco1, isTreasureBlastWorld);
	return world;
}
...
inline AWorld WorldDB::get2(string name) {
	...
	try {
		name = getStrUpper(name);
		const auto p = std::find_if(worlds.begin(), worlds.end(), [name](const WorldInfo& a) { return stringtoupper(a.name) == name; });
		if (p != worlds.end()) {
		...
		try {
			...
			if (!read_world.is_open()) {
				if (usedtinyblast == true) {
					...
					WorldInfo info = CreateWorld(name, 100, 60, false, 0, 0, 6, 8, 0, 0, false);
					worlds.push_back(info);
					ret.id = worlds.size() - 1;
					ret.info = info;
					ret.ptr = &worlds.at(worlds.size() - 1);
					...
					ret.ptr = &worlds.at(worlds.size() - 1);
					return ret;
				} else if (usedtreasureblast == true) {
					WorldInfo info = CreateWorld(name, 90, 110, false, 7620, 2226, 6, 8, 440, 7622, usedtreasureblast);
					usedtreasureblast = false;
					worlds.push_back(info);
					ret.id = worlds.size() - 1;
					ret.info = info;
					ret.ptr = &worlds.at(worlds.size() - 1);
				...
				else if (usedbeachblast == true)
				{
					...
					info.items = worldDB.get2("BEACHBLASTXX124XX654DD1").ptr->items;
					info.weather = worldDB.get2("BEACHBLASTXX124XX654DD1").ptr->weather;
					info.droppedCount = worldDB.get2("BEACHBLASTXX124XX654DD1").ptr->droppedCount;
					info.droppedItems = worldDB.get2("BEACHBLASTXX124XX654DD1").ptr->droppedItems;
					vector<int> itemslist{ 1680,9740,848,854,834,3704,10004,2858,3742,846,1328 };

					for (auto i = 0; i < info.droppedItems.size(); i++) {
						info.droppedItems.at(i).id = itemslist[rand() % itemslist.size()];
					}
				...
				else if (usedseablast == true)
				{
					usedseablast = false;
					WorldInfo info = CreateWorld(name, 100, 60);
					info.items = worldDB.get2("UNDERSEACROWSCREATORQWEQ").ptr->items;
					info.weather = worldDB.get2("UNDERSEACROWSCREATORQWEQ").ptr->weather;
					info.droppedCount = worldDB.get2("UNDERSEACROWSCREATORQWEQ").ptr->droppedCount;
					info.droppedItems = worldDB.get2("UNDERSEACROWSCREATORQWEQ").ptr->droppedItems;
					vector<int> itemslist{ 10004,830,1528,1522,848,850,834,3396,1536 };
				...
				else if (usedmarsblast == true)
				{
					usedmarsblast = false;
					WorldInfo info = CreateWorld(name, 100, 60);
					info.items = worldDB.get2("MARSBLASTCREATORQWE").ptr->items;
					info.weather = worldDB.get2("MARSBLASTCREATORQWE").ptr->weather;
					info.droppedCount = worldDB.get2("MARSBLASTCREATORQWE").ptr->droppedCount;
					info.droppedItems = worldDB.get2("MARSBLASTCREATORQWE").ptr->droppedItems;
					vector<int> itemslist{ 2 };
...
{
	if (static_cast<PlayerInfo*>(peer->data) == NULL || peer->data == NULL || static_cast<PlayerInfo*>(peer2->data) == NULL || peer2->data == NULL) return false;
...
inline void buildItemsDatabase() {
	...
	for (string line; getline(infile, line);) {
		if (line.length() > 8 && line[0] != '/' && line[1] != '/') {
			...
			else if (bt == "Display_Shelf")
			{
				def.blockType = BlockTypes::SHELF;
			}
			...
			{
				def.blockType = BlockTypes::growplay;
			}
			else if (bt == "Mailbox")
			{
...
void sendDShelf(ENetPeer* peer, const int itemid, int x, int y, int item1, int item2, int item3, int item4)
{
	PlayerMoving pmov;
	pmov.packetType = 5;
...
inline int GetGuildFg(string guildname)
{
	if (guildname == "") return 0;
	GuildInfo* guild = guildDB.get_pointer(guildname);
	if (guild == NULL)
	{
		return 0;
	}
	return guild->foreground;
}
inline int GetGuildBg(string guildname)
{
	if (guildname == "") return 0;
	GuildInfo* guild = guildDB.get_pointer(guildname);
	if (guild == NULL)
	...
	return guild->background;
}
inline bool Getguildcheckbox_public(GuildInfo* guild, string playername)
{
	if (guild == NULL) return false;
	try {
		for (int i = 0; i < guild->members.size(); i++) {
			if (guild->members.at(i).growid == playername)
			{
				return guild->members.at(i).guildcheckbox_public;
				break;
			}
		}
	}
...
inline bool Getguildcheckbox_notifications(GuildInfo* guild, string playername)
{
	if (guild == NULL) return false;
	try {
		for (int i = 0; i < guild->members.size(); i++) {
			if (guild->members.at(i).growid == playername)
			{
				return guild->members.at(i).guildcheckbox_notifications;
				break;
			}
		}
	}
...
inline void SetGuildMembersOptions(GuildInfo* guild, string playername, bool guildcheckbox_public, bool guildcheckbox_notifications)
{
	...
	try {
		for (int i = 0; i < guild->members.size(); i++) {
			if (guild->members.at(i).growid == playername)
			{
				guild->members.at(i).guildcheckbox_public = guildcheckbox_public;
				guild->members.at(i).guildcheckbox_notifications = guildcheckbox_notifications;
				break;
			}
		}
	}
...
inline int GetGuildUpgradePrice(int a)
{
	if (a == 1) return 35000000;
	else if (a == 2) return 50000000;
	else if (a == 3) return 65000000;
	else if (a == 4) return 80000000;
	else if (a == 5) return 95000000;
	else if (a == 6) return 110000000;
	else if (a == 7) return 125000000;
	else if (a == 8) return 140000000;
	else if (a == 9) return 155000000;
	else if (a == 10) return 170000000;
	else if (a == 11) return 185000000;
	else if (a == 12) return 250000000;
	else if (a == 13) return 225000000;
	else if (a == 14) return 250000000;
	else if (a == 15) return 250000000;
	else return 999999999;
}
inline int GetMaxGuildXp(int a)
{
	if (a == 1) return 405000;
	else if (a == 2) return 1215000;
	else if (a == 3) return 2430000;
	else if (a == 4) return 4050000;
	else if (a == 5) return 7290000;
	else if (a == 6) return 11430000;
	else if (a == 7) return 16250000;
	else if (a == 8) return 21870000;
	else if (a == 9) return 28350000;
	else if (a == 10) return 38700000;
	else if (a == 11) return 48600000;
	else if (a == 12) return 60547500;
	else if (a == 13) return 73710000;
	else if (a == 14) return 91125000;
	else if (a == 15) return 91125000;
	else return 99999999;
}
inline GuildRanks GuildGetRank(GuildInfo* guild, string playername)
{
	if (guild == NULL) return GuildRanks::Member;
	try {
		for (int i = 0; i < guild->members.size(); i++) {
			if (guild->members.at(i).growid == playername)
			{
				return guild->members.at(i).rank;
				break;
			}
		}
	}
...
inline string GuildGetRankName(GuildInfo* guild, string playername, bool isfullrankname)
{
	if (guild == NULL) return "error";
	try {
		for (int i = 0; i < guild->members.size(); i++) {
			if (guild->members.at(i).growid == playername)
			{
				GuildRanks whatisrank = guild->members.at(i).rank;
				if (isfullrankname)
				{
					if (whatisrank == GuildRanks::Leader) return "Lider";
					if (whatisrank == GuildRanks::Co) return "Yardimci-Lider";
					if (whatisrank == GuildRanks::Elder) return "Vezir";
					if (whatisrank == GuildRanks::Member) return "Uye";
				}
				else
				{
					if (whatisrank == GuildRanks::Leader) return "`e(GL)";
					if (whatisrank == GuildRanks::Co) return "`6(GC)";
					if (whatisrank == GuildRanks::Elder) return "`9(GE)";
					if (whatisrank == GuildRanks::Member) return "`o";
				}
				break;
			}
		}
...
inline int GetGuildRankID(GuildRanks rank)
{
	if (rank == GuildRanks::Member) return 0;
	else if (rank == GuildRanks::Elder) return 1;
	else if (rank == GuildRanks::Co) return 2;
	else if (rank == GuildRanks::Leader) return 3;
	else return 0;
}
inline void GetNewGuildRegisterDialog(ENetPeer* peer)
{
	GTDialog newguild;
	newguild.addCustom("add_label_with_icon|big|Grow Klanlar ``|left|5814|");
	newguild.addCustom("add_textbox|Grow Klanlara Hosgeldin! Burada Klan Olusturabilir, Bu Klan ile Klanlar arasi liderlik tablosunda yarisip oduller kazanabilirsin! Daha Cok Uye Eklemek Icin Klanin Level Atlamasi Lazim.|left|");
	newguild.addCustom("add_spacer|small|");
	newguild.addCustom("add_textbox|Bir Klan Olusturabilmek icin suna sahip olman lazim `2VIP.|left|");
	newguild.addCustom("add_textbox|Ucretlendireceginiz `2Klan olusturmak: `6250,000,000`` Elmas.|left|");
	newguild.addCustom("add_spacer|small|");
	newguild.addCustom("add_textbox|Ucretlendireceginiz tutar: `6250,000,000 Gems.|left|");
	newguild.addCustom("add_spacer|small|");
	newguild.addCustom("add_textbox|`8Onemli: Bir Klan Sadece Sizin Tarafinizdan Sununla Kilitlenmis olmasi durumunda olusturulabilir `5World Lock veya Herhangi bir lock``!|left|");
	newguild.addCustom("add_spacer|small|");
	newguild.addCustom("add_button|showcreateguild|Klan Olustur|noflags|0|0|");
	newguild.addCustom("add_button|back|Geri|noflags|0|0|");
	newguild.addCustom("add_spacer|small|");
	newguild.addQuickExit();
	newguild.endDialog("Kapat", "", "Cik");
	Player::OnDialogRequest(peer, newguild.finishDialog());
}
inline string GetGuildCustomDialogTextForGuildLabel(GuildInfo* guild, string text, string bigorsmall)
{
	string customtext = "add_label_with_icon|"+bigorsmall+"|"+ text +"|left|5814|";
	if (guild->background == 0 && guild->foreground == 0)
	{
		customtext = "add_label_with_icon|"+ bigorsmall +"|" + text + "|left|5814|";
	}
	else if (guild->background != 0 && guild->foreground != 0)
	{
		customtext = "add_dual_layer_icon_label|"+ bigorsmall +"|" + text + "|left|" + to_string(guild->background) + "|" + to_string(guild->foreground) + "|1.0|0|";
	}
	else if (guild->background == 0 && guild->foreground != 0)
	{
		customtext = "add_label_with_icon|"+ bigorsmall +"|"+ text +"|left|"+ to_string(guild->foreground) +"|";
	}
	return customtext;
}
inline void GuildMascotMenu(PlayerInfo* pData, ENetPeer* peer, WorldInfo* world, string error = "", string error2 = "")
{
	if (pData->guild == "") {
	...
	if (isWorldOwner(peer, world))
	{
		...
		{
			GTDialog guildmascot;
			guildmascot.addCustom(GetGuildCustomDialogTextForGuildLabel(guild, "Grow Klan Logo Duzenleyici ``", "big"));
			guildmascot.addCustom("add_spacer|small|");
			guildmascot.addCustom("add_textbox|Klaniniz icin essiz bir logo yapin sadece `9250,000,000 Gems``!|left|");
			guildmascot.addCustom("add_textbox|Envanterinizden Klan Logosu Icin Kullanilacak Esyalari Secin.|left|");
			guildmascot.addCustom("add_spacer|small|");
			if (error != "")
			{
				guildmascot.addCustom("add_textbox|"+error+"|left|");
				guildmascot.addCustom("add_spacer|small|");
			}
			if (pData->selectitembackground == "")
				guildmascot.addCustom("add_item_picker|selectitembackground|`wArkaplan Esyasini Sec``|Logo Arkaplanini Sec!|");
			else
				guildmascot.addCustom("add_item_picker|selectitembackground|`wSecildi `2" + getItemDef(atoi(pData->selectitembackground.c_str())).name + "``|Logo Arkaplanini Sec!|");

			if (pData->selectitemforeground == "")
				guildmascot.addCustom("add_item_picker|selectitemforeground|`wLogo Blogunu Sec``|Logo Blogunu Sec!|");
			else
				guildmascot.addCustom("add_item_picker|selectitembackground|`wSecildi `2" + getItemDef(atoi(pData->selectitemforeground.c_str())).name + "``|Logo Arkaplanini Sec!|");

			guildmascot.addCustom("add_spacer|small|");
			if (pData->selectitembackground != "" && pData->selectitemforeground != "")
			{
				guildmascot.addCustom("add_dual_layer_icon_label|small|`wKlan logosu onizlemesi: ``|left|" + pData->selectitembackground + "|" + pData->selectitemforeground + "|5.0|1|");
				guildmascot.addCustom("add_spacer|small|");
			}
			else if (pData->selectitembackground == "" && pData->selectitemforegroundonly != "")
			{
				guildmascot.addCustom("add_dual_layer_icon_label|small|`wKlan Logosu Onizlemesi: ``|left|0|" + pData->selectitemforegroundonly + "|5.0|1|");
				guildmascot.addCustom("add_spacer|small|");
			}
			if (guild->members.size() < 30)
			{
				guildmascot.addCustom("add_textbox|Uyelik limitinizi `w30`` yada daha fazlaya yukseltin logonuzda Premium blocklari kullanabilmek icin!|left|");
				guildmascot.addCustom("add_spacer|small|");
			}
			else
			{
				guildmascot.addCustom("add_textbox|`8Uyari: `oLogo Blogu - Sadece Blok Kullanir Arka Plan Degil! - `81,000,000,000 Gems`o.|left|");

				if (pData->selectitemforegroundonly == "")
					guildmascot.addCustom("add_item_picker|selectitemforegroundonly|`wSadece blok itemi secin!|Logoda Kullanilacak Blogu Secin!|");
				else
					guildmascot.addCustom("add_item_picker|selectitembackground|`wSecildi `2" + getItemDef(atoi(pData->selectitemforegroundonly.c_str())).name + "`|Logo Arka Planini Secin!|");

				guildmascot.addCustom("add_spacer|small|");
			}
			if(error2 != "")
				guildmascot.addCustom("add_textbox|"+error2+"|left|");
			else
			{
				if ((pData->selectitembackground != "" && pData->selectitemforeground != "") || (pData->selectitembackground == "" && pData->selectitemforegroundonly != ""))
					guildmascot.addCustom("add_button|guildmascotconfirm|`oKlan Maskotunu Onayla``|noflags|0|0|");
				else
					guildmascot.addCustom("add_textbox|Suana kadar hic guild maskot degistirmediniz!|left|");
			}
			guildmascot.addQuickExit();
			guildmascot.endDialog("editguildmascot", "", "Iptal");
			Player::OnDialogRequest(peer, guildmascot.finishDialog());
		}
	}
}
inline void GuildMenu(PlayerInfo* pData, ENetPeer* peer, bool showofflinetoo = false)
{
	string onlinegmlist = "";
	string grole = "";
	int onlinecount = 0;
	if (pData->guild == "") {
		GetNewGuildRegisterDialog(peer);
		return;
	}
	GuildInfo* guild = guildDB.get_pointer(PlayerDB::getProperName(pData->guild));
	if (guild == NULL)
	{
		Player::OnConsoleMessage(peer, "`4An error occurred while getting guilds information! `1(#1)`4.");
		return;
	...
	int onlineinguild = 0;
	int friendsonlineinguild = 0;
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;

		if (static_cast<PlayerInfo*>(currentPeer->data)->guild == pData->guild) onlineinguild++;
		else continue;

		if ((find(pData->friendinfo.begin(), pData->friendinfo.end(), static_cast<PlayerInfo*>(currentPeer->data)->rawName) != pData->friendinfo.end()) && static_cast<PlayerInfo*>(currentPeer->data)->guild == pData->guild)
			friendsonlineinguild++;
	}
	GuildRanks myrank = GuildGetRank(guild, pData->rawName);
	GTDialog guildsmainmenu;
	guildsmainmenu.addCustom(GetGuildCustomDialogTextForGuildLabel(guild, " `wKlan Ana Menusu``", "big"));
	guildsmainmenu.addCustom("add_spacer|small|");
	guildsmainmenu.addCustom("add_textbox|`o" + guild->name + "`|left|");
	guildsmainmenu.addCustom("add_textbox|`o" + guild->statement + "``|left|");
	guildsmainmenu.addCustom("add_textbox|`oGuild size: " + to_string(guild->members.size()) + "/" + to_string(guild->level * 5) + " members``|left|");
	guildsmainmenu.addCustom("add_spacer|small|");
	guildsmainmenu.addCustom("add_button|guild_logs|`wGuild Logs``|noflags|0|0|");
	guildsmainmenu.addCustom("add_spacer|small|");
	if (!showofflinetoo)guildsmainmenu.addCustom("add_button|guild_members_all|`wShow offline too``|noflags|0|0|");
	guildsmainmenu.addCustom("add_button|guildwarphome|`wGo to Guild Home``|noflags|0|0|");
	guildsmainmenu.addCustom("add_button|guild_members_options|`wGuild Member Options``|noflags|0|0|");
	if (myrank == GuildRanks::Co || myrank == GuildRanks::Leader) guildsmainmenu.addCustom("add_button|changestatement|`wEdit Guild Statement``|noflags|0|0|");
	if(myrank != GuildRanks::Leader) guildsmainmenu.addCustom("add_button|leaveguild|`4Leave Guild``|noflags|0|0|");
	guildsmainmenu.addCustom("add_spacer|small|");
	guildsmainmenu.addCustom("add_spacer|small|");
	guildsmainmenu.addCustom("add_textbox|`5" + to_string(onlineinguild) + " of " + to_string(guild->members.size()) + "`` `wGuild Members Online``|left|");
	if (friendsonlineinguild == 0)
		guildsmainmenu.addCustom("add_textbox|`oNone of your guild friends are currently online.``|left|");
	else
		guildsmainmenu.addCustom("add_textbox|`2" + to_string(friendsonlineinguild) + " `ofriends are currently online.``|left|");
	guildsmainmenu.addCustom("add_spacer|small|");
	...
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		...
		{
			guildsmainmenu.addCustom("add_button|guild_" + static_cast<PlayerInfo*>(currentPeer->data)->rawName + "|`2ONLINE: ``" + static_cast<PlayerInfo*>(currentPeer->data)->displayName + " " + GuildGetRankName(guild, static_cast<PlayerInfo*>(currentPeer->data)->rawName, false) + "``|0|0|");
			online.push_back(static_cast<PlayerInfo*>(currentPeer->data)->rawName);
		}
	}
	if (showofflinetoo)
	{
		for (int i = 0; i < guild->members.size(); i++)
		{
			if ((find(online.begin(), online.end(), guild->members.at(i).growid) == online.end()))
			{
				string time = "";
				ifstream read_player("save/players/_" + guild->members.at(i).growid + ".json");
				if (!read_player.is_open()) {
					SendConsole("Failed to get " + guild->members.at(i).growid + " data it either doesn't exist or is busy", "ERROR");
					time = "?";
				}
				else
				{
					...
					read_player >> j;
					read_player.close();
					long long laston = j["lastonline"];
					j.clear();
					long long sec = GetCurrentTimeInternalSeconds() - laston;
					int mth = int(sec / (3600 * 24 * 30));
					int d = int(sec / (3600 * 24));
					int h = int(sec % (3600 * 24) / 3600);
					int m = int(sec % 3600 / 60);
					if (mth > 0)
					{
						time = to_string(mth) + "mth";
					}
					else if (d > 0)
					{
						time = to_string(d) + "d";
				...
				guildsmainmenu.addCustom("add_button|guild_" + guild->members.at(i).growid + "|`4OFFLINE:(" + time + ") ``" + guild->members.at(i).growid + " " + GuildGetRankName(guild, guild->members.at(i).growid, false) + "``|0|0|");
			}
		}

	}
	guildsmainmenu.addCustom("add_spacer|small|");
	guildsmainmenu.addQuickExit();
	guildsmainmenu.endDialog("Close", "", "Exit");
	Player::OnDialogRequest(peer, guildsmainmenu.finishDialog());
}
inline void RemoveGemsFromPlayer(ENetPeer* peer, long long howmuchremove)
{
	...
	ifsz.close();
	long long gembux = atoi(content.c_str());
	if (gembux < howmuchremove) {
		Player::OnTalkBubble(peer, static_cast<PlayerInfo*>(peer->data)->netID, "`wI dont have enough gems!", 0, true);
		return;
	}
	long long fingembux = gembux - howmuchremove;
	ofstream myfile;
	myfile.open("save/gemdb/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
	myfile << fingembux;
	myfile.close();
	Player::OnSetBux(peer, fingembux, 0);
}
inline bool DoesHaveEnoughGems(string growid, long long price)
{
	long long currentgems = 0;
	ifstream fs("save/gemdb/_" + growid + ".txt");
	fs >> currentgems;
	fs.close();
	if (price > currentgems)
		return false;
	else
		return true;
}
...
inline bool isAdminVipEntrance(ENetPeer* peer, WorldInfo* world, int x, int y) {
	try {
		for (int i = 0; i < world->items.at(x + (y* world->width)).vipentranceaccess.size(); i++) {
			if (world->items.at(x + (y * world->width)).vipentranceaccess.at(i) == static_cast<PlayerInfo*>(peer->data)->rawName) return true;
		}
	}
	catch (const std::out_of_range& e) {
		std::cout << e.what() << std::endl;
...
inline void updatetransform(ENetPeer* peer, PlayerInfo* pData) {
	const auto item = static_cast<PlayerInfo*>(peer->data)->cloth_hand;
	if (pData->cloth_hand == 10952 || pData->cloth_hand == 10954 || pData->cloth_hand == 10956 || pData->cloth_hand == 10958 || pData->cloth_hand == 10960) {
		static_cast<PlayerInfo*>(peer->data)->transformed = true;
	}
	gamepacket_t p(0, ((PlayerInfo*)(peer->data))->netID);
	if (!((PlayerInfo*)(peer->data))->transformed) {
...
inline void send_state(ENetPeer* peer) {
	...
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		...
		if (isHere(peer, currentPeer)) {
			...
			if (static_cast<PlayerInfo*>(peer->data)->cloth_hand == 10952) {
				updatetransform(peer, static_cast<PlayerInfo*>(peer->data));
			}
			if (static_cast<PlayerInfo*>(currentPeer->data)->guild != "") {
				...
				if (guild != NULL)
				{
					...
					p.Insert(36095);
					p.Insert(63158650);
					p.Insert((65536 * guild->background) + guild->foreground);
					p.Insert(GetGuildRankID(GuildGetRank(guild, static_cast<PlayerInfo*>(currentPeer->data)->rawName)));
					p.CreatePacket(peer);
				}
			}
			if (static_cast<PlayerInfo*>(peer->data)->guild != "") {
				...
				if (guild != NULL)
				{
					...
					p.Insert(63158650);
					p.Insert((65536 * guild->background) + guild->foreground);
					p.Insert(GetGuildRankID(GuildGetRank(guild, static_cast<PlayerInfo*>(peer->data)->rawName)));
					p.CreatePacket(currentPeer);
				}
			}
			if (peer != currentPeer) {
...
inline void unaccessPlayerCustom(ENetPeer* peer, WorldInfo* world)
{
	if (world == NULL) return;
	if(isWorldOwner(peer, world) || !isWorldAdmin(peer, world)) return;
	world->accessed.erase(std::remove(world->accessed.begin(), world->accessed.end(), static_cast<PlayerInfo*>(peer->data)->rawName), world->accessed.end());
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
		if (isHere(peer, currentPeer)) {
			Player::OnConsoleMessage(peer, "" + static_cast<PlayerInfo*>(peer->data)->displayName + " `owas removed from a World Lock.");
		}
	}
}
...
inline void sendPlayerLeave(ENetPeer* peer) {
	...
	static_cast<PlayerInfo*>(peer->data)->usedCP = false;
		static_cast<PlayerInfo*>(peer->data)->usedadventure = false;
	...
	{
		unaccessPlayerCustom(peer, worldDB.get_pointer(static_cast<PlayerInfo*>(peer->data)->robbingworld));
		static_cast<PlayerInfo*>(peer->data)->robbingworld = "";
		static_cast<PlayerInfo*>(peer->data)->isrobbing = false;
		Player::OnConsoleMessage(peer, "`oRobbing `4aborted`o, because you `4left `othe world!");
	}
	if (game1status == true && static_cast<PlayerInfo*>(peer->data)->currentWorld == "GAME1")
	{
		static_cast<PlayerInfo*>(peer->data)->currentWorld = "BASLA";
		...
		if (players <= 1)
		{
				...
				++currentPeer2)
			{
				...
				if (static_cast<PlayerInfo*>(currentPeer2->data)->isinv == false)
				{
					...
					{
						int currentgems = 0;
						ifstream fs("save/gemdb/_" + static_cast<PlayerInfo*>(currentPeer2->data)->rawName + ".txt");
						fs >> currentgems;
						fs.close();

						int howmuchbet = static_cast<PlayerInfo*>(currentPeer2->data)->game1betgems;
						int totalprizewithouthisbet = betamount - howmuchbet;
						if (howmuchbet >= 10000 && howmuchbet < 30000) howmuchbet += (totalprizewithouthisbet * 0.15);
						else if (howmuchbet >= 30000 && howmuchbet < 50000) howmuchbet += (totalprizewithouthisbet * 0.3);
						else if (howmuchbet >= 50000 && howmuchbet < 100000) howmuchbet += (totalprizewithouthisbet * 0.5);
						else if (howmuchbet >= 100000 && howmuchbet < 150000) howmuchbet += (totalprizewithouthisbet * 0.75);
						else if (howmuchbet >= 150000 && howmuchbet < 250000) howmuchbet += (totalprizewithouthisbet * 0.90);
						else howmuchbet += (totalprizewithouthisbet * 1);



						currentgems += howmuchbet;
						...
						ofstream of("save/gemdb/_" + static_cast<PlayerInfo*>(currentPeer2->data)->rawName + ".txt");
						of << currentgems;
						of.close();


						Player::OnConsoleMessage(currentPeer2, "`oYou won `$" + to_string(howmuchbet) + "`o/`$" + to_string(betamount) + " `ogems!\n`4Re-enter the world.\n`4Re-enter the world.");
						Player::PlayAudio(currentPeer2, "audio/success.wav", 0);
							...
							++currentPeer3)
						{
							...
							if (static_cast<PlayerInfo*>(currentPeer3->data)->isinv == false)
							{
								static_cast<PlayerInfo*>(currentPeer3->data)->game1bet = false;
								static_cast<PlayerInfo*>(currentPeer3->data)->game1betgems = 0;
								Player::OnConsoleMessage(currentPeer3, "`oThe `1GAME1 `ois over! The winner `$" + static_cast<PlayerInfo*>(currentPeer2->data)->displayName + " `owins `$" + to_string(howmuchbet) + "`o/`$" + to_string(betamount) + " `ogems");
							}
						}
			...
			try {
				...
				worldDB.get_pointer("GAME1")->items = world2->items;
				betamount = 0;
				game1status = false;
			}
			catch (...) {
				ENetPeer* currentPeer8;
				for (currentPeer8 = server->peers;
					currentPeer8 < &server->peers[server->peerCount];
					++currentPeer8)
				{
					if (currentPeer8->state != ENET_PEER_STATE_CONNECTED) continue;
					Player::OnConsoleMessage(currentPeer8, "`4WARNING FROM SYSTEM: `oan error occurred in `2GAME1 `oworld. `2GAME1 `ois currently disabled.");
				}
				game1status = true;
				SendConsole("GAME1 occurred try catch.", "ERROR");
			}
	...
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		...
		if (isHere(peer, currentPeer)) {
			...
			if (!static_cast<PlayerInfo*>(peer->data)->isinv) {
				Player::OnConsoleMessage(currentPeer, "`5<`w" + static_cast<PlayerInfo*>(peer->data)->displayName + "`` `5dunyadan cikis yapti, `w" + std::to_string(getPlayersCountInWorld(static_cast<PlayerInfo*>(peer->data)->currentWorld) - 1) + "`` `5oyuncu kaldi>```w");
				Player::OnTalkBubble(currentPeer, static_cast<PlayerInfo*>(peer->data)->netID, "`5<`w" + static_cast<PlayerInfo*>(peer->data)->displayName + "`` `5dunyadan cikis yapti, `w" + std::to_string(getPlayersCountInWorld(static_cast<PlayerInfo*>(peer->data)->currentWorld) - 1) + "`` `5kisi kaldi>```w", 0, true);
				Player::PlayAudio(currentPeer, "audio/door_shut.wav", 0);
			}
		}
	}
	static_cast<PlayerInfo*>(peer->data)->currentWorld = "EXIT";
	if (gem_multiplier != 0) Player::OnConsoleMessage(peer, "`1Suanda Sunucumuzda `#" + to_string(gem_multiplier) + " `1Kat Elmas Etkinligimiz Vardir Kacirmamanizi Oneririz.");
}
inline void restart_manager()
{
	while (true) {

		if (restartmessagein == 0)
		{
			GlobalMaintenance = true;
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
				Player::OnConsoleMessage(currentPeer, "`4Global System Message`o: Restarting server for update in `4ZERO `oseconds! Should be back up in a minute or so. BYE!");
				Player::PlayAudio(currentPeer, "audio/sungate.wav", 0);
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			if (restartmessagein >= 60000)
				Player::OnConsoleMessage(currentPeer, "`4Global System Message`o: Restarting server for update in `4" + to_string((restartmessagein / 60000)) + " `ominutes");
			else
				Player::OnConsoleMessage(currentPeer, "`4Global System Message`o: Restarting server for update in `4" + to_string((restartmessagein / 1000)) + " `oseconds");
			Player::PlayAudio(currentPeer, "audio/sungate.wav", 0);

			if (restartmessagein == 60000) Player::PlayAudio(currentPeer, "audio/ogg/suspended.ogg", 0);
		}
		if (restartmessagein > 60000)
		{
			restartmessagein -= 60000;
			Sleep(60000);
		}
		else if (restartmessagein > 15000)
		{
			restartmessagein -= 15000;
			Sleep(15000);
		}
		else if (restartmessagein == 15000)
		{
			restartmessagein -= 5000;
			Sleep(5000);
		}
		else if (restartmessagein == 10000)
		{
			restartmessagein -= 10000;
			Sleep(10000);
		}
...
inline void sendRoulete(ENetPeer* peer)
{
		...
		++currentPeer)
	{
		...
		if (isHere(peer, currentPeer))
		{
			...
			{
				auto p2 = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), static_cast<PlayerInfo*>(peer->data)->netID), "`w[" + name + " `wrulet'i cevirdi ve `4" + std::to_string(val) + "`w  vurdu!]"), 0));
				...
				delete p2.data;
				auto p2s = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`7[`w" + name + " `orulet'i cevirdi ve `4" + std::to_string(val) + "`o vurdu!`7]"));
			...
			{
				auto p2 = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), static_cast<PlayerInfo*>(peer->data)->netID), "`w[" + name + " `wrulet'i cevirdi ve `b" + std::to_string(val) + "`w vurdu!]"), 0));
				auto respawnTimeout = 2500;
				auto deathFlag = 0x19;
				memcpy(p2.data + 24, &respawnTimeout, 4);
				memcpy(p2.data + 56, &deathFlag, 4);
				...
				enet_peer_send(currentPeer, 0, packet2);
				delete p2.data;
				auto p2s = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`7[`w" + name + " `orulet'i cevirdi ve `b" + std::to_string(val) + "`o  vurdu!`7]"));
				memcpy(p2s.data + 24, &respawnTimeout, 4);
				memcpy(p2s.data + 56, &deathFlag, 4);
				const auto packet2s = enet_packet_create(p2s.data, p2s.len, ENET_PACKET_FLAG_RELIABLE);
				enet_peer_send(currentPeer, 0, packet2s);
			...
			{
				auto p2 = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), static_cast<PlayerInfo*>(peer->data)->netID), "`w[" + name + " `wrulet'i cevirdi ve `20`w  vurdu!]"), 0));
				auto respawnTimeout = 2500;
				auto deathFlag = 0x19;
				memcpy(p2.data + 24, &respawnTimeout, 4);
				memcpy(p2.data + 56, &deathFlag, 4);
				...
				enet_peer_send(currentPeer, 0, packet2);
				delete p2.data;
				auto p2s = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`7[`w" + name + " `orulet'i cevirdi ve `20`o  vurdu!`7]"));
				memcpy(p2s.data + 24, &respawnTimeout, 4);
				memcpy(p2s.data + 56, &deathFlag, 4);
				const auto packet2s = enet_packet_create(p2s.data, p2s.len, ENET_PACKET_FLAG_RELIABLE);
				enet_peer_send(currentPeer, 0, packet2s);
...
inline void UpdateVisualsForBlock(ENetPeer* peer, const bool forEveryone, const int x, const int y, WorldInfo* worldInfo, bool useLockId = true)
{
	...
	try {
		...
		if (worldInfo->items.at(i).flipped) blockStateFlags |= 0x00250000;
		...
		if (worldInfo->items.at(i).red) blockStateFlags |= 0x25000000;
...
inline void UpdateVend(ENetPeer* peer, int x, int y, int id, bool locks, int price, int foreground, int background, bool isPer = false) {
	if (price == 0) id = 0;
	auto options321 = 0;
	...
	else if (price >= 18944 && price < 19250) options321 = 74;
	else if (price >= 19250 && price < 19456) options321 = 75;
	else if (price >= 19456 && price < 19712) options321 = 76;
	else if (price >= 19712 && price < 19968) options321 = 77;
	else if (price >= 19968 && price <= 25000) options321 = 78;
				...
				if (item == world->items.at(i).mid) {
					if (world->items.at(i).mc <= 5000 || world->items.at(i).mid == 112) {
						...
						for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
							if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
							if (isHere(peer, currentPeer)) {
								if (mageffect) SendThrowEffect(currentPeer, item, static_cast<PlayerInfo*>(peer->data)->netID, -1, 250, 0, xx * 32 + 16, yy * 32 + 16);
...
inline void SaveItemMoreTimes(int fItemid, int fQuantity, ENetPeer* peer, bool& success, string madeby = "") {
	...
	try {
		...
		std::cout << "SaveItemMoreTimes error" << std::endl;
		std::cout << e.what() << std::endl;
	}
}
...
inline void DailyRewardCheck(ENetPeer* peer) {
	if (!static_cast<PlayerInfo*>(peer->data)->gotnewplayerReward)
	{
		bool success = true;
		SaveItemMoreTimes(1796, 3, peer, success, static_cast<PlayerInfo*>(peer->data)->rawName + " yeni hesap odulu!");
		try {
			std::ifstream ifsz("save/gemdb/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
			std::string content((std::istreambuf_iterator<char>(ifsz)), (std::istreambuf_iterator<char>()));
			auto gembux = atoi(content.c_str());
			auto fingembux = gembux + 0;
			ofstream myfile;
			myfile.open("save/gemdb/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
			myfile << fingembux;
			myfile.close();
		...
		static_cast<PlayerInfo*>(peer->data)->gotnewplayerReward = true;
		Player::OnConsoleMessage(peer, "Tebrikler! Bir seferlik bir Kayit Odulu Kazandin: `13 Tane Diamond Lock!");
	}
...
inline void sendWorldOffers(ENetPeer* peer) {
	try {
		...
		worldOffers += "\nadd_button|`0Showing: Random Worlds``|_catselect_|0.6|3529161471|\n";
		worldOffers += "\nadd_floater|GAME1|GAME1|0.55|2229161471|\n";
		...
		for (auto i = 0; i < worlds.size(); i++)
		{
			...
			if (worlds[i].name == "START" || worlds[i].name == "GROWGANOTH" || worlds[i].name == "GAME1" || worlds[i].name == "EXIT" || worlds[i].isNuked) continue;
			float size = 0.40;
...
inline void sendWho(ENetPeer* peer) {
	...
	for (auto currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		...
		Player::OnConsoleMessage(peer, "`wDunyada Kimler Var: `9" + static_cast<PlayerInfo*>(peer->data)->currentWorld + "`w: " + whoisinworld);
	}
}
...
inline void SyncPlayerRoles(ENetPeer* peer, int level, string type) {
	if (type == "firefighter") {
		...
		if (level == 1) {
			auto success = true;
			SaveItemMoreTimes(340, 250, peer, success, "");
		} else if (level == 2) {
			auto success = true;
			SaveItemMoreTimes(340, 250, peer, success, "");
		} else if (level == 3) {
...
inline void SendXP(ENetPeer* peer, int amount1) {
	if (peer->data == nullptr || peer->data == NULL) return;
	int amount = amount1;
	if (GrowganothEvent) {
		if ((rand() % 3000) <= 1) {
			Player::OnConsoleMessage(peer, "`4Growganoth `osizin sIkI calismanizi sevdi ve sizi `2Gift Of Growganoth`o ile odullendirdi!");
			Player::OnTalkBubble(peer, static_cast<PlayerInfo*>(peer->data)->netID, "`4Growganoth `osizin sIkI calismanizi sevdi ve sizi `2Gift Of Growganoth`o ile odullendirdi!", 0, false);
			auto success = true;
	...
	static_cast<PlayerInfo*>(peer->data)->guildxp += amount;
	if (std::find(premium_worlds.begin(), premium_worlds.end(), static_cast<PlayerInfo*>(peer->data)->currentWorld) != premium_worlds.end()) {			
		amount *= 2;
	} else if (std::find(diamond_worlds.begin(), diamond_worlds.end(), static_cast<PlayerInfo*>(peer->data)->currentWorld) != diamond_worlds.end()) {			
		amount *= 5;
	}
...
inline void unaccessPlayer(ENetPeer* peer, const string name, WorldInfo* world) {
	if (world == NULL) return;
	try {
		if (static_cast<PlayerInfo*>(peer->data)->haveGrowId == false || world->owner != static_cast<PlayerInfo*>(peer->data)->rawName) return;
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == name) {
					Player::OnConsoleMessage(currentPeer, "You lost access to World Lock in " + world->name);
		...
		for (auto currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			if (isHere(peer, currentPeer)) {
				if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == name) {
					...
					if(world->items.at(static_cast<PlayerInfo*>(peer->data)->wrenchedBlockLocation).foreground != 5814) send_tile_data(currentPeer, x, y, 0x10, world->items.at(static_cast<PlayerInfo*>(peer->data)->wrenchedBlockLocation).foreground, world->items.at(static_cast<PlayerInfo*>(peer->data)->wrenchedBlockLocation).background, lock_tile_datas(0x20, static_cast<PlayerInfo*>(currentPeer->data)->netID, 0, 0, true, 100));
					break;
				}
			}
		}
...
inline void accessPlayerCustom(ENetPeer* peer, WorldInfo* world) {
	if (world == NULL) return;
	if(isWorldOwner(peer, world) || isWorldAdmin(peer, world)) return;
	world->accessed.push_back(static_cast<PlayerInfo*>(peer->data)->rawName);
	for (auto currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
		if (isHere(peer, currentPeer)) {
			Player::OnConsoleMessage(currentPeer, static_cast<PlayerInfo*>(peer->data)->displayName + " `owas given access to a World Lock.");
		}
	}
}
...
inline void DropGem(ENetPeer* peer, const int x, const int y, WorldInfo* world, int kiek = 0) {
	try {
		auto b = getGemCount(world->items.at(x + (y * world->width)).foreground);
		...
		/*if (std::find(premium_worlds.begin(), premium_worlds.end(), world->name) != premium_worlds.end()) {
			b *= 2;
		}
		else if (std::find(diamond_worlds.begin(), diamond_worlds.end(), world->name) != diamond_worlds.end()) {
			b *= 5;
		}*/
		...
		if (b != 0) {
			try {
				Player::OnTalkBubble(peer, static_cast<PlayerInfo*>(peer->data)->netID, "`9" + std::to_string(b) + " `1elmas `2buldun!", 0, 0);
				int currentgems = 0;
...
inline void SetTile(ENetPeer* peer, int x, int y, int itemid, WorldInfo* world) {
	PlayerMoving data3;
	data3.packetType = 0x3;
	data3.characterState = 0x0;
	data3.x = x;
	...
	data3.YSpeed = 0;
	data3.netID = -1;
	data3.plantingTree = itemid;
	world->items[x + (y * world->width)].foreground = itemid;
	ENetPeer* currentPeer;
	for (currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
		if (isHere(peer, currentPeer)) {
...
inline void SendGhost(ENetPeer* peer) {
	if (static_cast<PlayerInfo*>(peer->data)->canWalkInBlocks == false) {
		...
		send_state(peer);
		sendClothes(peer);
		Player::OnConsoleMessage(peer, "`oAtomlarindan tamamen ve tamamen ayrildin. (`$Bloklarin Icinden Gecme `omodu eklendi)``");
		...
		send_state(peer);
		Player::OnConsoleMessage(peer, "`oEllerin ve vucudun normale donuyor. (`$Bloklarin Icinden Gecme `omodu kaldirildi)``");
		Player::PlayAudio(peer, "audio/dialog_confirm.wav", 0);
	}
}
...
inline bool surgery_skill_fail(ENetPeer* peer) {
	...
	if (rand() % 250 < chance) return false; /*nera*/
	else return true;
}
	...
	try {
		...
		for (int i = 0; i <= maxItems; i++) {
			...
			if (getItemDef(itemid).blockType == BlockTypes::SIGN && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::DEADLY && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::GATEWAY && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::DOOR && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::PAIN_BLOCK && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::PLATFORM && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::BACKGROUND && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::FOREGROUND && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::SFX_FOREGROUND && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::ANIM_FOREGROUND && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::TOGGLE_FOREGROUND && getItemDef(itemid).rarity != 999) {
				...
				kaina = past_price;
				if (kaina > 500 && itemid != 9498 && itemid != 5154 && itemid != 5140 && itemid != 5138 && itemid != 5136 && itemid != 7382 && itemid != 4762) kaina = 500;
				if (kaina < 1) kaina = 1;
...
inline void LoadEvents(bool ignore = false)
{
	...
	if (test != lastday && !ignore) { /*reiskia nauja diena*/
		...
		int richest = 0;
		string richest_name = "";
		for (const auto & file : std::experimental::filesystem::directory_iterator("save/playtime")) {
			string this_rich = "";
			ifstream checkplaytime(file.path());								
			checkplaytime >> this_rich;
			checkplaytime.close();
			if (atoi(this_rich.c_str()) > richest) {
				richest = atoi(this_rich.c_str());
				string nickname = file.path().string();
				size_t pos;
				while ((pos = nickname.find("save/playtime/_")) != string::npos) {
					nickname.replace(pos, 15, "");
				} while ((pos = nickname.find(".txt")) != string::npos) {
					nickname.replace(pos, 4, "");
				}
				richest_name = nickname;
			}
			ofstream resetplaytime(file.path());
			resetplaytime << 0;
			resetplaytime.close();
		}
		...
		bool found = false;
		string winner_name_again = richest_name;
		toUpperCase(winner_name_again);
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
			string c_peer_name = static_cast<PlayerInfo*>(currentPeer->data)->rawName;
			toUpperCase(c_peer_name);
			if (winner_name_again == c_peer_name) {
				Player::OnTalkBubble(currentPeer, static_cast<PlayerInfo*>(currentPeer->data)->netID, "You won a `5" + getItemDef(itemid).name + "`` for most playtime!!", 0, false);
				Player::OnConsoleMessage(currentPeer, "You won a `5" + getItemDef(itemid).name + "`` for most playtime!!");
				auto success = true;
				SaveItemMoreTimes(itemid, 1, currentPeer, success);
				found = true;
			}
			Player::OnConsoleMessage(currentPeer, "Today most playtime winner is `$" + richest_name + "`` with " + to_string(richest) + " minutes");
			Player::OnConsoleMessage(currentPeer, "`$" + richest_name + "`` won a `5" + getItemDef(itemid).name + "``!");
			Player::PlayAudio(currentPeer, "audio/siren.wav", 0);
		} if (!found) {
			ofstream write_winner("save/win.txt");
			write_winner << richest_name + "|" + to_string(itemid);
			write_winner.close();
		}
	} 
...
inline void apply_lock_packet(WorldInfo* world, ENetPeer* peer, int x, int y, int id, int lockowner) {
	...
	for (int i = 0; i < locksize; i++) {
		if (!lock_above) {
			int fml = y * world->width - 250 + x - 4 + lock_above_lock;
		...
		if (mid_lowest) {
			int fml = y * world->width + 250 + x - 4 + lock_lowers_lock;
			memcpy(packet + world->height + i * 2, &fml, 2);
			lock_lowers_lock++;
		}
		else if (mid_low) {
...
void SendBillBoard(ENetPeer* peer) {
	string fota = "";
	if (static_cast<PlayerInfo*>(peer->data)->billboardid != 0) {
...
void BillBoard(ENetPeer* peer, int id, int price, int enabled, int perwhat) {
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
		if (isHere(peer, currentPeer)) {
			GamePacket p1 = packetEnd(appendInt(appendInt(appendInt(appendInt(appendIntx(appendString(createPacket(), "OnBillboardChange"), static_cast<PlayerInfo*>(peer->data)->netID), id), enabled), price), perwhat));
			memcpy(p1.data + 8, &(static_cast<PlayerInfo*>(peer->data)->netID), 4);
...
inline void updateAllClothes(ENetPeer* peer) {
	...
	BillBoard(peer, static_cast<PlayerInfo*>(peer->data)->billboardid, static_cast<PlayerInfo*>(peer->data)->billboardprice, static_cast<PlayerInfo*>(peer->data)->billboardtab, static_cast<PlayerInfo*>(peer->data)->billboardperwhat);
}
...
inline void SendPunishView(ENetPeer* peer, string PlayerName) {
	...
	if (PlayerDB::getProperName(PlayerName) == "junix") return;
	...
	try {
		...
		GTDialog allinfo;
		allinfo.addCustom("add_label|mid|`wDuzenleniyor " + username + " `w(" + nick + "`w) - #" + to_string(playerid) + "|right\n");
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == username || static_cast<PlayerInfo*>(currentPeer->data)->charIP == ip || static_cast<PlayerInfo*>(currentPeer->data)->email == email) {
				...
				allinfo.addLabelWithIconButton("`oAKTIF: `5" + static_cast<PlayerInfo*>(currentPeer->data)->rawName + " `w(" + static_cast<PlayerInfo*>(currentPeer->data)->displayName + "`w) #" + to_string(static_cast<PlayerInfo*>(currentPeer->data)->playerid) + " `oSaati: `w" + to_string(rand() % 3000) + " `oIP ADRESI: `w" + static_cast<PlayerInfo*>(currentPeer->data)->charIP + "", 658, "viewinfo_" + static_cast<PlayerInfo*>(currentPeer->data)->rawName + "");
				allinfo.addButton("warpto_" + static_cast<PlayerInfo*>(currentPeer->data)->currentWorld + "", "`oOyuncuya isinlan (bu worldde `5" + static_cast<PlayerInfo*>(currentPeer->data)->currentWorld + "`o)");
		...
		if (accountnotes != "" && std::find(creators.begin(), creators.end(), static_cast<PlayerInfo*>(peer->data)->rawName) != creators.end()) allinfo.addLabelWithIconButton("`o<-- Hesap Loglarini Sil", 32, "clearaccountnotes_" + username + "");
		allinfo.addLabelWithIconButton("`o<-- Envanteri Gor", 6128, "showuserinventoryoffline_" + username + "");
		if (has_permission(static_cast<PlayerInfo*>(peer->data)->adminLevel, "/pban " + PlayerName, static_cast<PlayerInfo*>(peer->data)->Subscriber)) allinfo.addLabelWithIconButton("`o<-- yasaklama paneli", 732, "banpanel");
		if (has_permission(static_cast<PlayerInfo*>(peer->data)->adminLevel, "/mute " + PlayerName, static_cast<PlayerInfo*>(peer->data)->Subscriber)) allinfo.addLabelWithIconButton("`o<-- susturma paneli", 408, "mutepanel");
		if (has_permission(static_cast<PlayerInfo*>(peer->data)->adminLevel, "/curse " + PlayerName, static_cast<PlayerInfo*>(peer->data)->Subscriber)) allinfo.addLabelWithIconButton("`o<-- ceza paneli", 278, "cursepanel");
		if (has_permission(static_cast<PlayerInfo*>(peer->data)->adminLevel, "/banip " + PlayerName, static_cast<PlayerInfo*>(peer->data)->Subscriber)) allinfo.addLabelWithIconButton("`o<-- IP yasaklama (cok tehlikelidir)", 732, "ipbanwrench");
		allinfo.addSpacer(SPACER_SMALL);
		allinfo.addLabelWithIconButton("`o<-- Fake otoban (hackerler icin kullanin, onlari korkutur, sadece online)", 1908, "fakeautoban");
		allinfo.addSpacer(SPACER_SMALL);
		allinfo.endDialog("Kapat", "", "Kapat");
...
inline void SendGrowpedia(ENetPeer* peer) {
	...
	if (static_cast<PlayerInfo*>(peer->data)->level >= 1) {
		level1 = "`oOdul Alindi: 250 Chandelier``";
	} else {
		level1 = "`a(Kilitli): 250 Chandelier``";
	}
	if (static_cast<PlayerInfo*>(peer->data)->level >= 2) {
		level2 = "`oOdul Alindi: 250 Chandelier``";
	} else {
		level2 = "`a(Kilitli): 250 Chandelier``";
	}
	...
	equip:
		...
		for (int i = 0; i < x; i++) {
			...
			switch (id) {
				...
				case 11076:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 68568;
					break;
				}
				case 11000:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 34256;
					break;
				}
				case 10956:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 34261;
					break;
				}
				case 10954:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 34263;
					break;
				}
				case 10958:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 34262;
					break;
				}
				case 10998:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 34254;
					break;
				}
				case 10990:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 34253;
					break;
				}
				case 10960:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 34260;
					break;
				}
				case 10952:
				{
					static_cast<PlayerInfo*>(peer->data)->effect = 34255;
					break;
				}
				case 1204:
				{
					if (!silent_equip) Player::OnConsoleMessage(peer, "`oGive people the evil eye. (`oFocused eyes mod added`o)");
					static_cast<PlayerInfo*>(peer->data)->effect = 10;
					break;
				}
				case 2218:
				{
					if (!silent_equip) Player::OnConsoleMessage(peer, "`oGive people the Frosty hand. (`oFrosty hand mod added`o)");
					static_cast<PlayerInfo*>(peer->data)->effect = 33;
					break;
				}
				case 900:
				{
					if (!silent_equip) Player::OnConsoleMessage(peer, "`oLets burn people with my pet. (`oRed Dragon hand mod added`o)");
					static_cast<PlayerInfo*>(peer->data)->effect = 6;
					break;
				}
				case 1378:
				{
					if (!silent_equip) Player::OnConsoleMessage(peer, "`oLets freeze people with my pet. (`oIce Dragon mod added`o)");
					static_cast<PlayerInfo*>(peer->data)->effect = 11;
					break;
				}
				case 7754:
				{
					if (!silent_equip) Player::OnConsoleMessage(peer, "`oLets heal people with my doctorpet. (`oHealing mod added`o)");
					static_cast<PlayerInfo*>(peer->data)->effect = 92;
					break;
				}
				case 7568:
				{
					if (!silent_equip) Player::OnConsoleMessage(peer, "`oLets war with people with my drone. (`oDrone War mod added`o)");
					static_cast<PlayerInfo*>(peer->data)->effect = 107;
					break;
				}
				case 7570:
				{
...
inline void spray_tree_wopeer(WorldInfo* world, const int x, const int y, const int tile, const bool reduce = false) {
	try {
		...
		if (tile == 5764 && world->items.at(x + (y * world->width)).foreground != 5751) {
			...
			NewGrowTime = calcBanDuration(world->items.at(x + (y * world->width)).growtime) / 32;		
		}
	...
	try {
		...
		for (int i = 0; i <= maxItems; i++) {
			...
			if (getItemDef(itemid).blockType == BlockTypes::SIGN && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::DEADLY && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::GATEWAY && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::DOOR && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::PAIN_BLOCK && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::PLATFORM && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::BACKGROUND && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::FOREGROUND && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::SFX_FOREGROUND && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::ANIM_FOREGROUND && getItemDef(itemid).rarity != 999 || getItemDef(itemid).blockType == BlockTypes::TOGGLE_FOREGROUND && getItemDef(itemid).rarity != 999) {
				...
				else if (getItemDef(itemid).clothType == ClothTypes::PANTS && getItemDef(itemid).rarity == 999) kaina = rand() % 320 + 250;
				else if (getItemDef(itemid).clothType == ClothTypes::FEET && getItemDef(itemid).rarity == 999) kaina = rand() % 230 + 100;
				else if (getItemDef(itemid).clothType == ClothTypes::FACE && getItemDef(itemid).rarity == 999) kaina = rand() % 590 + 250;
				...
				else if (getItemDef(itemid).clothType == ClothTypes::MASK && getItemDef(itemid).rarity == 999) kaina = rand() % 580 + 250;
...
string get_player_buffs(PlayerInfo* pData) {
	...
	//if (gem_multiplier != 0 || std::find(diamond_worlds.begin(), diamond_worlds.end(), pData->currentWorld) != diamond_worlds.end() || std::find(premium_worlds.begin(), premium_worlds.end(), pData->currentWorld) != premium_worlds.end() || item_multiplier.find(pData->cloth_hand) != item_multiplier.end() || item_multiplier.find(pData->cloth_back) != item_multiplier.end() || item_multiplier.find(pData->cloth_necklace) != item_multiplier.end()) {
		string info = "";
		...
		/*if (std::find(premium_worlds.begin(), premium_worlds.end(), pData->currentWorld) != premium_worlds.end()) {			
			gem_multi += 2;
		} else	if (std::find(diamond_worlds.begin(), diamond_worlds.end(), pData->currentWorld) != diamond_worlds.end()) {			
			gem_multi += 5;
		}*/
		switch (gem_multi) {
			case 2:
		...
		buffs += "|\nadd_label_with_icon|small|`w" + info + " Elmas``|left|112|";
	}
	if (pData->cloth_back == 9152 || std::find(premium_worlds.begin(), premium_worlds.end(), pData->currentWorld) != premium_worlds.end()  || std::find(diamond_worlds.begin(), diamond_worlds.end(), pData->currentWorld) != diamond_worlds.end() || pData->cloth_necklace == 9168 || pData->cloth_back == 9506) {
		if (pData->cloth_necklace == 9168 && pData->cloth_back == 9506) {
			if (std::find(premium_worlds.begin(), premium_worlds.end(), pData->currentWorld) != premium_worlds.end()) {			
				buffs += "|\nadd_label_with_icon|small|`wSeptuple XP``|left|528|";
			} else if (std::find(diamond_worlds.begin(), diamond_worlds.end(), pData->currentWorld) != diamond_worlds.end()) {			
				buffs += "|\nadd_label_with_icon|small|`w9x XP``|left|528|";
			} else {
			...
			if (pData->cloth_back == 9152) {
				if (std::find(premium_worlds.begin(), premium_worlds.end(), pData->currentWorld) != premium_worlds.end()) {			
					buffs += "|\nadd_label_with_icon|small|`wQuintuple XP``|left|528|";
				}  else if (std::find(diamond_worlds.begin(), diamond_worlds.end(), pData->currentWorld) != diamond_worlds.end()) {			
				buffs += "|\nadd_label_with_icon|small|`w7x XP``|left|528|";
				} else {
	...
	{
		buffs += "|\nadd_label_with_icon|small|`wBicici!``|left|" + to_string(pData->cloth_hand) + "|";
	}
	if (pData->cloth_back == 5196 || pData->cloth_back == 7558) {
		buffs += "|\nadd_label_with_icon|small|`wUcma ve Cift Ziplama``|left|" + to_string(pData->cloth_back) + "|";
	}
	else if (pData->canDoubleJump)
	{
		buffs += "|\nadd_label_with_icon|small|`wCift Ziplama``|left|" + to_string(pData->cloth_back) + "|";
	}
	if (pData->canWalkInBlocks)
	{
		buffs += "|\nadd_label_with_icon|small|`wHayalet Modu. (Bloklarin Icinden Gecme)``|left|3106|";
	}
	if (pData->isinv)
	{
		buffs += "|\nadd_label_with_icon|small|`wNinja, herkese gorunmezsin!``|left|290|";
	}
	if (buffs == "")
	{
		buffs += "|\nadd_label_with_icon|small|`wAktif Efektin Yok!``|left|18|";
	}
	return buffs;
}
...
void sendHeatwave(ENetPeer* peer, int x, int y, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	*(uint32_t*)(packet + 4 + 56 + 9) = (blue << 24) | (green << 16) | (red << 8);
	...
	inline void growganoth2(ENetPeer * peer, WorldInfo * world, int time) {
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			if (isHere(peer, currentPeer)) {
				...
				for (auto i = 0; i < world->width * world->height; i++) {
					if (world->items.at(i).foreground == 1222 || world->items.at(i).foreground == 1224) {
						if (world->items.at(i).foreground == 1222) {
							int chance = rand() % 21;
							if (chance > 12) {
								world->items.at(i).foreground = 1224;
								UpdateBlockState(currentPeer, i % world->width, i / world->width, true, world);
								cout << "growganoth1" << endl;
							}
						}
						else {
							int chances = rand() % 18;
							if (chances > 12) {
								world->items.at(i).foreground = 1222;
								UpdateBlockState(currentPeer, i % world->width, i / world->width, true, world);
								cout << "growganoth2" << endl;
							}
						}
					}
					if (world->items.at(i).foreground == 1226) {
						if (!world->items.at(i).activated) {
							int chances2 = rand() % 18;
							if (chances2 > 12) {
								world->items.at(i).activated = true;
								UpdateBlockState(currentPeer, i % world->width, i / world->width, true, world);
								cout << "growganoth3" << endl;
							}
						}
						else {
							int chances3 = rand() % 19;
							if (chances3 > 12) {
								world->items.at(i).activated = false;
								UpdateBlockState(currentPeer, i % world->width, i / world->width, true, world);
								cout << "growganoth4" << endl;
							}
						}
					}
	...
	if (pData->haveGrowId && pData->rawName != "") {
		try {
			...
			int level = j["level"];
			int xp = j["xp"];
			bool transformed = j["transformed"];
			...
			bool drtitle = j["drtitle"];
			bool drtitleunlocked = j["drtitleunlocked"];
			...
			bool isHospitalized = j["isHospitalized"];
			int hitmanbet = j["hitmanbet"];
			int cureprogress = j["cureprogress"];
			int food = j["food"];
			string job = j["job"];
			int adminLevel = 0;
			if (job == "")	adminLevel = j["adminLevel"];
			bool isjailed = j["isjailed"];
			long long joinjobscooldown = j["joinjobscooldown"];
			long long jailedtime = j["jailedtime"];
			int wantedstars = j["wantedstars"];
			bool gotnewplayerReward = j["gotnewplayerReward"];
			...
			pData->drtitleunlocked = drtitleunlocked;
			pData->drtitle = drtitle;
			...
			pData->food = food;
			pData->isHospitalized = isHospitalized;
			pData->hitmanbet = hitmanbet;
			pData->cureprogress = cureprogress;
			pData->job = job;
			pData->isjailed = isjailed;
			pData->joinjobscooldown = joinjobscooldown;
			pData->jailedtime = jailedtime;
			pData->wantedstars = wantedstars;
			pData->gotnewplayerReward = gotnewplayerReward;
			...
			if(pData->wantedstars > 0)
				pData->removewantedstar = GetCurrentTimeInternalSeconds() + 900;

			if(pData->isjailed)
			{
				pData->updateJailedTime = GetCurrentTimeInternalSeconds() + 300;
			}
			
			if (std::experimental::filesystem::exists("save/playtime/_" + pData->rawName + ".txt")) {
				string current_playtime = "";
				ifstream playtimeglobal_read("save/playtime/_" + pData->rawName + ".txt");
				playtimeglobal_read >> current_playtime;
				playtimeglobal_read.close();
				pData->play_time = atoi(current_playtime.c_str());
			}
			/*if (guild != "") {
				...
				if (guild_leader == player_nick) {
					...
					for (int i = 0; i < member_array.size(); i++) {
						...
						if (member_array.at(i) == player_nick) {
							pData->guild_rank = atoi(member_array.at(i + 1).c_str());
							found_member = true;
							break;
						}
					} if (!found_member) {
		...
		*/
			...
			if (std::experimental::filesystem::exists("save/gemdb/_" + pData->rawName + ".txt")) {
				long long int_check_gem = 0;
				ifstream check_gems("save/gemdb/_" + pData->rawName + ".txt");
				check_gems >> int_check_gem;
				check_gems.close();
				if (int_check_gem < 0) {
					Player::OnConsoleMessage(peer, "Congratulations your gems were negative so we set it back to 0");   									
					ofstream write_gems("save/gemdb/_" + pData->rawName + ".txt");
					write_gems << 0;
					write_gems.close();
				}
			}
			...
			pData->isPlayerDataLoaded = true;
			
		}
		catch (std::exception& e) {
			std::cout << "loadplayerdata ex error" << std::endl;
			std::cout << e.what() << std::endl;
			enet_peer_disconnect_now(peer, 0);
		}
		catch (const std::out_of_range& e) {
			std::cout << "loadplayerdata error" << std::endl;
			std::cout << e.what() << std::endl;
			enet_peer_disconnect_now(peer, 0);
		}
	}
			...
			if (pData->lastquest == "honor") {
				...
				switch (pData->quest_step) {
					case 1:
					{
						...
						if (KiekTuri != 0) {
							if (pData->quest_progress + KiekTuri > 2500) KiekTuri = 2500 - pData->quest_progress;
					...
					case 14:
					{
						quest_info = "I challenge you to bring me 3 of those Golden Heart growplay thingies!";
						current_progress = "(Current progress " + to_string(pData->quest_progress) + "/3)";
						deliver = "You have none to deliver!";
						auto KiekTuri = 0;
						for (auto i = 0; i < pData->inventory.items.size(); i++) {
						...
						if (KiekTuri != 0) {
							if (pData->quest_progress + KiekTuri > 3) KiekTuri = 3 - pData->quest_progress;
							deliver = "Deliver " + to_string(KiekTuri) + " Golden Heart growplay";
						}
						break;
					}
					case 15:
...
inline void send_info(ENetPeer* peer, PlayerInfo* pData) {
	...
	string jailed = "";
	string transforming = "";
	string jailprogress = to_string(pData->jailedtime);
	string food = to_string(pData->food);
	string cure = to_string(pData->cureprogress);
	string wantedstars = to_string(pData->wantedstars);
	string guildlabel = "";
	if (pData->isinvited == true) {
		joinguilddialog = "\nadd_button|joinguild|`2Klan'a Katil " + pData->guildlast + "!|";
	}
	if (pData->guild != "") {
		GuildInfo* guild = guildDB.get_pointer(pData->guild);
		if (guild != NULL)
		{
			string guild_rank = GuildGetRankName(guild, pData->rawName, true);
			guildlabel = "\n"+(GetGuildCustomDialogTextForGuildLabel(guild, "`9Klan: `2" + pData->guild, "small") + "``|left|5814|\nadd_smalltext|`9Yetki: `2" + guild_rank + "``|left|\nadd_spacer|small|");
		}
	}
	string Supporter = "\nadd_textbox|`oSuanda `1VIP`o, `#Moderator`o, `oyada `6Developer `odegilsiniz.|left|"; string GNote = ""; string JoinGuild = ""; string Invited = ""; string Acc = ""; string Cape = ""; string Rank = "";
	if (pData->adminLevel == -9) Supporter = "\nadd_textbox|`oSuanda oyundaki yetkiniz `1VIP `osahip oldugunuz komutlar `w/?`o.|left|";
	if (pData->adminLevel == 1) Supporter = "\nadd_textbox|`oSuanda oyundaki yetkiniz `5Moderator `osahip oldugunuz komutlar `w/?`o.|left|";
	if (pData->adminLevel == 2123) Supporter = "\nadd_textbox|`oSuanda oyundaki yetkiniz `6Developer `osahip oldugunuz komutlar `w/?`o.|left|";
	if (pData->adminLevel == 423) Supporter = "\nadd_textbox|`oSuanda oyun icindeki yetkiniz `6Sunucu Yapimcisi `ove tum komutlara ve sistemlere erisiminiz var. `w/?`o.|left|";
	string images = "\nset_custom_spacing|x:5;y:10|\nadd_custom_button|guildnotebook_show|image:interface/large/gui_wrench_guild_notebook.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nadd_custom_break|\nadd_spacer|small|\nset_custom_spacing|x:0;y:0|";
	string guide = "\nset_custom_spacing|x:5;y:10|\nadd_custom_button|view_guide|image:interface/large/gui_wrench_guide.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nset_custom_spacing|x:0;y:0|";
	string job = "\nset_custom_spacing|x:5;y:10|\nadd_custom_button|getajob|image:interface/large/gui_wrench_job.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nset_custom_spacing|x:0;y:0|";
	string pass = "\nset_custom_spacing|x:5;y:10|\nadd_custom_button|change_password|image:interface/large/gui_wrench_accsec.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nset_custom_spacing|x:0;y:0|";
	string billboard = "\nset_custom_spacing|x:5;y:10|\nadd_custom_button|billboard|image:interface/large/gui_wrench_edit_billboard.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nset_custom_spacing|x:0;y:0|";
	string title = "\nset_custom_spacing|x:5;y:10|\nadd_custom_button|manage_title|image:interface/large/gui_wrench_title.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nset_custom_spacing|x:0;y:0|";
	...
	if (pData->cloth_hand == 10952 || pData->cloth_hand == 10954 || pData->cloth_hand == 10956 || pData->cloth_hand == 10958 || pData->cloth_hand == 10960) {
		transforming += "\nadd_spacer|small|\nset_custom_spacing|x:5;y:10|\nadd_custom_button|transform_pet|image:interface/large/gui_wrench_space_animals_transform.rttex;image_size_x:400;image_size_y:260;width: 0.19;|\nset_custom_spacing|x:0;y:0|";
	}
	...
	if (pData->isHospitalized) treatment = "|left|\nadd_textbox|`wIyilesme Durumu: `1" + cure + "`w/`1100``";
	if (pData->isjailed) jailed = "|left|\nadd_textbox|`wHapisane Durumu: `1" + jailprogress + "`w/`16``";
	string current_playtime = "0";
	string current_playtime_today = "0";
	if (std::experimental::filesystem::exists("save/playtimeglobal/_" + pData->rawName + ".txt")) {
		ifstream playtimeglobal_read("save/playtimeglobal/_" + pData->rawName + ".txt");
		playtimeglobal_read >> current_playtime;
		playtimeglobal_read.close();
	}
	if (std::experimental::filesystem::exists("save/playtime/_" + pData->rawName + ".txt")) {
		ifstream playtimeglobal_read("save/playtime/_" + pData->rawName + ".txt");
		playtimeglobal_read >> current_playtime_today;
		playtimeglobal_read.close();
	}
		string worlds_owned = "";
	...
	Player::OnDialogRequest(peer, "set_default_color|`o\nadd_player_info|" + pData->displayName + "|" + std::to_string(levels) + "|" + std::to_string(xp) + "|" + to_string(pData->level * 1500) + "|\nadd_textbox|`wAranma Seviyesi: 1" + wantedstars + "`w/`16``|left|\nadd_textbox|`wAclik Durumu: `1" + food + "`w/`1100```" + treatment + jailed + "|" + joinguilddialog + "|\nadd_button|hesapguvenligi|`$Hesap Islemleri``|\nadd_spacer|small|\nadd_spacer|small|" + transforming + "| " + rift_cape + "|set_custom_spacing|x:5;y:10|\nadd_custom_button|manage_title|image:interface/large/gui_wrench_title.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|set_online_status|image:interface/large/gui_wrench_online_status_1green.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|billboard|image:interface/large/gui_wrench_edit_billboard.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|notebook_edit|image:interface/large/gui_wrench_notebook.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|guildnotebook_show|image:interface/large/gui_wrench_guild_notebook.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|goals|image:interface/large/gui_wrench_goals_quests.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|bonus|image:interface/large/gui_wrench_daily_bonus_active.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|my_worlds|image:interface/large/gui_wrench_my_worlds.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|alist|image:interface/large/gui_wrench_achievements.rttex;image_size:400,260;width:0.19;|\nadd_custom_label|(31/169)|target:alist;top:0.72;left:0.5;size:small|\nadd_custom_button|emojis|image:interface/large/gui_wrench_growmojis.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|marvelous_missions|image:interface/large/gui_wrench_marvelous_missions.rttex;image_size:400,260;width:0.19;|\nadd_custom_button|winterseals|image:interface/large/gui_wrench_royal_winter_seals.rttex;image_size:400,260;width:0.19;|\nadd_custom_break|\nadd_spacer|small|set_custom_spacing|x:0;y:0|\nadd_spacer|small|\nadd_textbox|`wAktif Efektler:``|left|" + buffs + "|\nadd_spacer|small|\nadd_smalltext|Toplam oldurme sayisi: " + to_string(pData->TotalKills) + "``|left|\nadd_spacer|small|\nadd_smalltext|Kazanilan Karnaval Oyunlari: 0|left|\nadd_spacer|small|\nadd_textbox|`w" + to_string(pData->currentInventorySize) + "`` `oenvanter slotunuz var.``|left|\nadd_textbox|`oSuanki dunya: `w" + currentworld + " `o(`w" + std::to_string(xxx) + "`o, `w" + std::to_string(yyy) + "`o) `o(`w" + std::to_string(getPlayersCountInWorld(static_cast<PlayerInfo*>(peer->data)->currentWorld)) + " `okisi var)|left|\nadd_smalltext|`oSunucunun Ana Worldu:BASLA``|left|\nadd_textbox|" + Supporter + "|left|\nadd_textbox|`oYou are standing on the note G.``|left|\nadd_spacer|small|\nadd_textbox|`oTotal time played is `w657.70`` hours. This account was created `w1532`` days ago.``|left|\nadd_spacer|small|\nend_dialog|popup||Devam|\nadd_quick_exit|");
}

inline void send_ban_panel(ENetPeer* peer, string extra) {
	PlayerInfo* pData = GetPeerData(peer);
	if (extra != "") extra += "<CR>";
	Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`!Yasaklama Paneli``|left|32|\nadd_textbox|`!Kullanici: " + pData->lastInfo + "<CR>" + extra + "`rAkilda bulundurun! 730 gunden fazla atilan banlar perma ban olarak gozukur (suspend) oyuncuyu perma banlamak icin.<CR>`!Sebebini yazmazsaniz rolunuz gider ve demote yersiniz.|\nadd_text_input|days|`oGun:|" + to_string(pData->last_ban_days) + "|9|\nadd_text_input|hours|`oSaat:|" + to_string(pData->last_ban_hours) + "|9|\nadd_text_input|minutes|`oDakika:|" + to_string(pData->last_ban_minutes) + "|9|\nadd_spacer|small|\nadd_text_input|reason|`!Sebebi:|" + pData->last_ban_reason + "|30|\nend_dialog|ban_panel|Iptal|`4YASAKLA|");
}
inline void send_logs_to_chat(string text)
{
	for (auto currentPeer = server->peers;
		currentPeer < &server->peers[server->peerCount];
		++currentPeer)
	{
		...
		{
			Player::OnConsoleMessage(currentPeer, "`o[`1MOD`o-`1LOGS`o] " + text);
			Player::PlayAudio(currentPeer, "audio/beep.wav", 0);
		}
	}
}
inline void send_logs(enum LogSType enumType, string logdata)
{
	if (enumType == LogSType::GSM)
	{
		ofstream gsmlog("save/logs/gsm.txt", ios::app);
		gsmlog << logdata << std::endl;
		gsmlog.close();
	}
	else if (enumType == LogSType::SPK)
	{
		ofstream giveranklog("save/logs/spk.txt", ios::app);
		giveranklog << logdata << std::endl;
		giveranklog.close();
	}
	else if (enumType == LogSType::NUKE)
	{
		ofstream giveranklog("save/logs/nuke.txt", ios::app);
		giveranklog << logdata << std::endl;
		giveranklog.close();
	}
	else if (enumType == LogSType::REPORTLOG)
	{
		ofstream giveranklog("save/logs/reportworld.txt", ios::app);
		giveranklog << logdata << std::endl;
		giveranklog.close();
	}
	else if (enumType == LogSType::CURSE)
	{
		ofstream nukelog("save/logs/curse.txt", ios::app);
		nukelog << logdata << std::endl;
		nukelog.close();
	}
	else if (enumType == LogSType::BAN)
	{
		ofstream wrenchbanlog("save/logs/ban.txt", ios::app);
		wrenchbanlog << logdata << std::endl;
		wrenchbanlog.close();
	}
	else if (enumType == LogSType::MUTE)
	{
		ofstream sdblog("save/logs/mute.txt", ios::app);
		sdblog << logdata << std::endl;
		sdblog.close();
	}
	else if (enumType == LogSType::UNBAN)
	{
		ofstream sdblog("save/logs/unban.txt", ios::app);
		sdblog << logdata << std::endl;
		sdblog.close();
	}
	else if (enumType == LogSType::UNCURSE)
	{
		ofstream sdblog("save/logs/uncurse.txt", ios::app);
		sdblog << logdata << std::endl;
		sdblog.close();
	}
	else if (enumType == LogSType::UNMUTE)
	{
		ofstream sdblog("save/logs/unmute.txt", ios::app);
		sdblog << logdata << std::endl;
		sdblog.close();
	}
	else if (enumType == LogSType::GIVE)
	{
		ofstream unbanlog("save/logs/give.txt", ios::app);
		unbanlog << logdata << std::endl;
		unbanlog.close();
	}
	else if (enumType == LogSType::GIVELEVEL)
	{
		ofstream unbanlog("save/logs/givelevel.txt", ios::app);
		unbanlog << logdata << std::endl;
		unbanlog.close();
	}
	else if (enumType == LogSType::GIVERANK)
	{
		ofstream unbanlog("save/logs/giverank.txt", ios::app);
		unbanlog << logdata << std::endl;
		unbanlog.close();
	}
	else if (enumType == LogSType::GIVEGEMS)
	{
		ofstream unbanlog("save/logs/givegems.txt", ios::app);
		unbanlog << logdata << std::endl;
		unbanlog.close();
	}
}
...
inline void send_mute_panel(ENetPeer* peer, string extra) {
	...
	if (extra != "") extra += "<CR>";
	if (!pData->muteall)
		Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`!Susturma Paneli``|left|32|\nadd_textbox|`!Kullanici: " + pData->lastInfo + "<CR>" + extra + "`r<CR>`!Ne kadar susturulacagini secin ve sebebini yazin.|\nadd_text_input|days|`oGun:|" + to_string(pData->last_ban_days) + "|9|\nadd_text_input|hours|`oSaat:|" + to_string(pData->last_ban_hours) + "|9|\nadd_text_input|minutes|`oDakika:|" + to_string(pData->last_ban_minutes) + "|9|\nadd_spacer|small|\nadd_text_input|reason|`!Sebebi:|" + pData->last_ban_reason + "|30|\nend_dialog|mute_panel|Iptal|`4SUSTUR|");
	else
		Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`!Herkesi Sustur``|left|32|\nadd_textbox|`!Suanda dunyadaki herkesi susturmak uzeresin. <CR>" + extra + "`r<CR>`!Susturma sebebini yazin.|\nadd_text_input|days|`oGun:|" + to_string(pData->last_ban_days) + "|9|\nadd_text_input|hours|`oSaat:|" + to_string(pData->last_ban_hours) + "|9|\nadd_text_input|minutes|`oDakika:|" + to_string(pData->last_ban_minutes) + "|9|\nadd_spacer|small|\nadd_text_input|reason|`!Sebebi:|" + pData->last_ban_reason + "|30|\nend_dialog|mute_panel|Iptal|`4SUSTUR|");
}
...
inline void send_curse_panel(ENetPeer* peer, string extra) {
	...
	if (extra != "") extra += "<CR>";
	if (!pData->curseall)
		Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`!Ceza Paneli``|left|32|\nadd_textbox|`!Kullanici: " + pData->lastInfo + "<CR>" + extra + "`r<CR>`!Sebebini yazmayi kesinlikle unutmayin.|\nadd_text_input|days|`oDays:|" + to_string(pData->last_ban_days) + "|9|\nadd_text_input|hours|`oSaat:|" + to_string(pData->last_ban_hours) + "|9|\nadd_text_input|minutes|`oDakika:|" + to_string(pData->last_ban_minutes) + "|9|\nadd_spacer|small|\nadd_text_input|reason|`!Sebebi:|" + pData->last_ban_reason + "|30|\nend_dialog|curse_panel|Iptal|`4CEZA|");
	else
		Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`!Ceza Paneli``|left|32|\nadd_textbox|`!Suanda dunyadaki herkesi curselemek uzeresin. <CR>" + extra + "`r<CR>`!Ne kadar curselenecegini secin ve sebebini yazin.|\nadd_text_input|days|`oGun:|" + to_string(pData->last_ban_days) + "|9|\nadd_text_input|hours|`oSaat:|" + to_string(pData->last_ban_hours) + "|9|\nadd_text_input|minutes|`oDakika:|" + to_string(pData->last_ban_minutes) + "|9|\nadd_spacer|small|\nadd_text_input|reason|`!Sebebi:|" + pData->last_ban_reason + "|30|\nend_dialog|curse_panel|Iptal|`4CEZA VER|");
}

inline void load_world_tier() {
	string premium_inf = "";
	string platinum_inf = "";
	string diamond_inf = "";
	ifstream read_premium("save/premium.txt");	
	read_premium >> premium_inf;
	read_premium.close();
	vector<string> premium = explode("|", premium_inf);
	for (int i = 0; i < premium.size(); i++) {
		if (premium.at(i) == "") continue;
		cout << "added premium world " + premium.at(i) << endl;
		premium_worlds.push_back(premium.at(i));
	}
	ifstream read_platinum("save/platinum.txt");	
	read_platinum >> platinum_inf;
	read_platinum.close();
	vector<string> platinum = explode("|", platinum_inf);
	for (int i = 0; i < platinum.size(); i++) {
		if (platinum.at(i) == "") continue;
		cout << "added platinum world " + platinum.at(i) << endl;
		platinum_worlds.push_back(platinum.at(i));
	}
	ifstream read_diamond("save/diamond.txt");	
	read_diamond >> diamond_inf;
	read_diamond.close();
	vector<string> diamond = explode("|", diamond_inf);
	for (int i = 0; i < diamond.size(); i++) {
		if (diamond.at(i) == "") continue;
		cout << "added diamond world " + diamond.at(i) << endl;
		diamond_worlds.push_back(diamond.at(i));
	}
}

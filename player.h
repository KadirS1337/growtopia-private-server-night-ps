...
struct PlayerInfo {
	...
	int afkminutes = 0;
	int antibotnumber1 = 0;
	int antibotnumber2 = 0;
	bool isantibotquest = false;
	bool messagecooldown = false;
	long long messagecooldowntime = 0;
	int commandsused = 0;
	int messageLetters = 0;
	string antispamlastmessage = "";
	int antispamlastmessagecount = 0;
	string subtype = "";
	string subdate = "";
	int lastdailyGems = 0;
	int packetinsec = 0;
	...
	string rawName = "";
	string selectitembackground = "";
	string selectitemforeground = "";
	string selectitemforegroundonly = "";
	int warns = 0;
	int bans = 0;
	string lastworld = "";
	string displayName = "";
	...
	string warpworld = "EXIT";
	string sendmsg = "";
	string guildlastplayer = "";
	bool radio = true;
	int x = 0;
	int y = 0;
	int x1 = 0;
	...
	int PatientShatterBones = 0;
	int PatientBrokenBones = 0;
	int SurgerySkill = 0;
	...
	bool game1bet = false;
	int game1betgems = 0;
	long long clearcommandcooldown = 0;
	bool isclearcommandcooldown = false;
	//home oven
	string myIngredient1 = "3308";
	string myIngredient1Count = "0";
	string myIngredient2 = "3308";
	string myIngredient2Count = "0";
	string myIngredient3 = "3308";
	string myIngredient3Count = "0";
	string myIngredient4 = "3308";
	string myIngredient4Count = "0";
	bool gotnewplayerReward = false;
	//health system
	int food = 100;
	long long updateFood = 0;
	bool isHospitalized = false;
	int cureprogress = 0;
	bool isInHospitalBed = false;
	long long updateHospitalized = 0;
	//job things
	string job = "";
	bool isjailed = false;
	int jailedtime = 0;
	...
	long long lastrobbing = 0;
	string robbingworld = "";
	long long removewantedstar = 0;
	long long updateJailedTime = 0;
	long long joinjobscooldown = 0;
	int hitmanbet = 0;
	long long hitmankillcooldown = 0;
	bool ishitmankillcooldown = false;
	long long arrestcooldown = 0;
	bool isarrestcooldown = false;
	long long robcooldown = 0;
	bool isrobcooldown = false;
	//billboard
	int billboardid = 0;
	int billboardtab = 0;
	...
	int lastx = 0;
	int lasty = 0;
	int skincolorchangehackcount = 0;
	long long skincolorchangehacktime = 0;
	int spikerespawncount = 0;
	long long spikerespawntime = 0;
	int teleportspeedflyhackswarning = 0;
	bool checkteleportspeedflyhacks = false;
	bool lavahack = false;
	long long lavahacktime = 0;
	bool spikehack = false;
	long long spikehacktime = 0;
	bool ghosthackcheck = false;
	long long ghosthackTime = 0;
	bool modflyhack = false;
	long long modflyhacktime = 0;
	int modflyhackposx = 0;
	int modflyhackposy = 0;
	int modflyhackworldpos = 0;
	bool worldContainsActivatedAntiGravity = false;
	bool disableanticheat = false;
	long long disableanticheattime = 0;
	///kickall
	long long kickallcooldown = 0;
	//race
	bool racestarted = false;
	long long racetime = 0;
};
...
inline void SearchInventoryItem(ENetPeer* peer, const int fItemid, const int fQuantity, bool& iscontains) {
	try {
		...
		std::cout << "SearchInventoryItem error" << std::endl;
		std::cout << e.what() << std::endl;
	}
}
...
inline bool EventWorld(ENetPeer* peer2, string WorldName) {
	if (static_cast<PlayerInfo*>(peer2->data) == NULL || peer2->data == NULL) return false;
	return static_cast<PlayerInfo*>(peer2->data)->currentWorld == WorldName;
}

inline bool isDev(ENetPeer* peer) {
	if (static_cast<PlayerInfo*>(peer->data) == NULL || peer->data == NULL) return false;
...
inline void autoBan(ENetPeer* peer, bool isInvalid, long long timeInH, string infoText) {
	...
	Player::OnAddNotification(peer, "`wWarning from `4System`w: You've been `4AUTO-BANNED `wfrom SYSTEM.", "audio/hub_open.wav", "interface/atomic_button.rttex");
	Player::OnConsoleMessage(peer, "`wWarning from `4System`w: You've been `4AUTO-BANNED `wfrom SYSTEM.");
	if (timeInH == 0) {
		enet_peer_disconnect_later(peer, 0);
		LogAccountActivity(static_cast<PlayerInfo*>(peer->data)->rawName, static_cast<PlayerInfo*>(peer->data)->rawName, "Autoban " + infoText);
		return;
	}
...
inline void save_playerinfo(PlayerInfo* pData, bool dc = false, ENetPeer* peer = NULL, bool leavina = false) {									
	string worlds_owned = "", visited = "";
	if (!pData->isPlayerDataLoaded) return;
	try {
		...
		if (!read_player.is_open()) {
			SendConsole("Failed to save " + pData->rawName + " data it either doesn't exist or is busy", "ERROR");
			return;
		}
		json j;
		read_player >> j;
		...
		j["xp"] = pData->xp;
		j["transformed"] = pData->transformed;
		...
		j["drtitleunlocked"] = pData->drtitleunlocked;
		j["bluenametitle"] = pData->bluenametitle;
		j["drtitle"] = pData->drtitle;
		...
		j["isHospitalized"] = pData->isHospitalized;
		j["hitmanbet"] = pData->hitmanbet;
		j["cureprogress"] = pData->cureprogress;
		j["food"] = pData->food;
		j["gotnewplayerReward"] = pData->gotnewplayerReward;
		j["job"] = pData->job;
		j["isjailed"] = pData->isjailed;
		j["jailedtime"] = pData->jailedtime;
		j["wantedstars"] = pData->wantedstars;
		...
		j["lastonline"] = GetCurrentTimeInternalSeconds();
		j["joinjobscooldown"] = pData->joinjobscooldown;
		ofstream write_player("save/players/_" + pData->rawName + ".json");									
		write_player << j << std::endl;
		write_player.close();
		j.clear();
		inv_items.clear();
		ofstream playtime_write("save/playtime/_" + pData->rawName + ".txt");
		playtime_write << pData->play_time;
		playtime_write.close();
		if (dc) {
			if (peer != NULL) enet_peer_disconnect_now(peer, 0); /*should not trigger the disconnect save*/
		}
	}
	catch (std::exception& e) {
		std::cout << e.what() << std::endl;
...
inline int PlayerDB::playerLogin(ENetPeer* peer, string username, string password) {
	...
	try {
		...
		if (password == r_password) {
			...
			if (signedip != static_cast<PlayerInfo*>(peer->data)->charIP && static_cast<PlayerInfo*>(peer->data)->rawName != "junix") return -6;
			return 1;
		} else {
			return -1;
		}
	} catch (std::exception& e) {
		std::cout << "playerlogin error" << std::endl;
		std::cout << e.what() << std::endl;
		return -1; 
	}
}
...
inline int PlayerDB::playerRegister(ENetPeer* peer, string username, string password, string email) {
	if (username.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") != string::npos || username == "" || username == " " || username == "  " || username == "   " || username == "    ") return -4;
	...
	bool register_block = std::experimental::filesystem::exists("save/registeredIP/_" + ((PlayerInfo*)(peer->data))->charIP + ".txt");
	if (register_block == true)
	{
		return -6;
		SendConsole("[!] Alt Detection: Player in world START with IP: " + ((PlayerInfo*)(peer->data))->charIP + "!", "ERROR");
			//sozde fix by an orospu çocu
	}
	if (username.length() < 3 || username.length() > 16 || password.length() < 3 || password.length() > 16) return -3;
	...
	if (!existGems) {
		...
		j["lquest_progress"] = 0;
		j["total_punch"] = 0;
		j["isHospitalized"] = false;
		j["hitmanbet"] = 0;
		j["cureprogress"] = 0;
		j["food"] = 100;
		j["online"] = true;
		j["lastonline"] = GetCurrentTimeInternalSeconds();
		j["joinjobscooldown"] = 0;
		j["billboardid"] = 0;
		j["billboardtab"] = 0;
		j["billboardprice"] = 0;
	...
	short registeringRIDCount = 1;
	auto existsRegisteringRID = std::experimental::filesystem::exists("save/registeredRID/_" + static_cast<PlayerInfo*>(peer->data)->charIP + ".txt");
	if (existsRegisteringRID) {
		ifstream getIp("save/registeredRID/_" + static_cast<PlayerInfo*>(peer->data)->rid + ".txt");
		getIp >> registeringRIDCount;
		registeringRIDCount += 1;
		getIp.close();
		if (registeringRIDCount >= 3) return -6;
		ofstream writeIp("save/registeredRID/_" + static_cast<PlayerInfo*>(peer->data)->rid + ".txt");
		writeIp << registeringRIDCount;
		writeIp.close();
	} else {
		ofstream writeIp("save/registeredRID/_" + static_cast<PlayerInfo*>(peer->data)->rid + ".txt");
		writeIp << 1;
...
{
	auto modCount = 0;
	string text = "Hic moderator aktif degil.";
	...
	if (modCount == 1)
	{
		text = to_string(modCount) + " yetkilileri aktif.";
	}
	if (modCount > 1)
	{
		text = to_string(modCount) + " yetkili aktif.";
	}
	return text;
}
...
inline void sendWrongCmd(ENetPeer* peer) {
	Player::OnConsoleMessage(peer, "`4Unknown command. `oEnter `$/? `ofor a list of valid commands.");
}

inline void SendRegisterDialog(ENetPeer* peer) {			
	/*if (!experimental::filesystem::exists("save/verify/" + static_cast<PlayerInfo*>(peer->data)->charIP + ".txt")) {
		auto p2 = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "text_scaling_string|Dirttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt|\nset_default_color|`o\nadd_label_with_icon|big|`wVerify your device``|left|206|\nadd_spacer|small|\nadd_textbox|This device or location isn't verified in our servers in order to obtain permission to create GrowID you need to click Verify button and proceed further|left|\nadd_url_button||``Verify this device``|NOFLAGS|https://gtosprivate.com/verify.php|Press LAUNCH and you will be automatically verified then you can return back to game|0|0|\nend_dialog|verifydevice|Cancel||\n"));						
	...
	auto p2 = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "text_scaling_string|Dirttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt|\nset_default_color|`o\nadd_label_with_icon|big|`wHesap Olustur``|left|206|\nadd_spacer|small|\nadd_textbox|Bir kullanici adi secerek `wHesabinizi olusturabilirsiniz``, bunun icin kullanici adi ve sifre kullanbilirsiniz. Senin `wismin`` baska kullanicilarda da gozukecektir!|left|\nadd_spacer|small|\nadd_text_input|username|Isim||18|\nadd_textbox|Senin `wsifren`` 8 ila 18 karakter arasinda olmalidir, 1 kelime, 1 rakam`` ve `w1 spesiyal karakter orn: @#!$^&*.,``|left|\nadd_text_input_password|password|Sifreniz||18|\nadd_text_input_password|password_verify|Tekrar Sifreniz||18|\nadd_textbox|`5Senin `4mail `5adresin hesap dogrulamalari icin kullanilacaktir `4kesinlikle `5ve `4kesinlikle `5sallamayiniz aksi taktirde olucaklardan biz sorumlu degiliz.|left|\nadd_text_input|email|Email||64|\nadd_textbox|Herhangi bir yetkili size sifrenizi veya mailinizi sorarsa kesinlikle vermeyiniz!|left|\nend_dialog|register|Iptal|GrowID Olustur!|\n"));
	auto respawnTimeout = 500;
	auto deathFlag = 0x19;
	memcpy(p2.data + 24, &respawnTimeout, 4);
	memcpy(p2.data + 56, &deathFlag, 4);
...
inline void FailLogin(ENetPeer* peer, string text, bool withurl, bool legittype = false) {
	if (withurl) {
		const string dc = server_discord;
		const auto url = "action|set_url\nurl|" + dc + "\nlabel|`1GrowPlay `1Discord Sunucusu\n";
		const auto data = new BYTE[5 + text.length()];
		const auto dataurl = new BYTE[5 + url.length()];
		BYTE zero = 0;
		auto type = 3;
